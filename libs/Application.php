    ...
    public function get($xml)
    {
        ...
        foreach ($xml->item as $application)
        {
            ...
            foreach ($application->questions->question as $question)
            {
                ...
                // Hacks for multiple checkboxes. We don't want to require *every* checkbox be checked.
                // If the <inputs> had been setup properly we could use the minlength validator of jquery-validate,
                // but instead we'll invent our own.
                if ($required && strtoupper($question['type']) == 'CHECKBOXES')
                {
                    $numCheckboxes = substr_count($form, '{class}');

                    // Remove the "required" class
                    $myClasses = array_diff($myClasses, array('required'));

                    // Add a class based on the field id
                    $myClasses[] = 'ccms-field-' . $question['id'];

                    if ($numCheckboxes > 0)
                    {
                        // Replace all but the last class
                        $form = preg_replace("/\{class\}/", implode(' ', $myClasses), $form, $numCheckboxes - 1);

                        // Replace the last class and include 'one-required' class.
                        $form = str_replace('{class}', implode(' ', $myClasses) . ' one-required', $form);
                    }
                }
                else
                {
